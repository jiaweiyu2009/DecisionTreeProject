...
def getEntropy(classCountTuple):
    class0 = classCountTuple[0] + 1
    class1 = classCountTuple[1] + 1
    class2 = classCountTuple[2] + 1
...
def run_train_test(training_data, training_labels, testing_data):
    ...
    modified_train_data = training_data
    for x in range(len(training_data)):
        modified_train_data[x].append(training_labels[x])

    trained_tree = grow_tree(modified_train_data)

    predict_label_list = [0] * len(testing_data)
    for x in range(len(testing_data)):
        predict_label_list[x] = predict_data(
            testing_data[x], trained_tree)
...
def run_train_test_2(training_data, training_labels, testing_data):
    modified_train_data = training_data
    for x in range(len(training_data)):
        modified_train_data[x].append(training_labels[x])

    trained_tree = grow_tree(modified_train_data)
    return predict_data(testing_data, trained_tree)
